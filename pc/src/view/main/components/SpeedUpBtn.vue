<template>
  <div class="ljb-index-left" :class="{'ljb-index-left-animation':isShow}">
    ...
    <div class="btn-container">
      ...
      <button :disabled="isDisabled" class="restart" @click.stop.prevent="restartFn()" v-show="isShowRestartBtn">
    ...
    <div class="btn-cover" :class="{'display-block': isShowBtnCover}"></div>
     <!-- :class="{'display-block': !isShowBtnCover}" -->
  </div>
</template>
...
export default {
  ...
  data: function() {
    return {
      ...
      isLoginOkToAccOkStatus: true, // 是从login_ok状态变为acc_ok状态
      currentMode: null,
      currentPattern: null,
      delayClickRestart: true
    }
  },
  methods: {
            ...
            if (data.ret === 0) { // 非accing状态
              if (this.userConflict === 0 || this.userConflict === 8 || this.accingUserConflict === 0) {
                ...
                if (this.accOkStatus === 'ACC_OK') { // 加速成功的状态
                  ...
                  this.isShowSpeedUpPng = false // 隐藏旋转图片
                  this.$root.Bus.$emit('busShowCover', false)
                  this.isLoginOkToAccOkStatus = true
                  let aa = setTimeout(() => {
                    this.isShowBtnCover = false // 隐藏遮罩层
                    clearTimeout(aa)
                  }, 500)
    ...
    restartFn: function() {
      ...
      if (this.delayClickRestart) {
        ...
        this.txtSpeedBtn = '正在启动加速'
        this.isShowBtnCover = true // 显示遮罩层
        this.delayClickRestart = false
      }
      let aa = setTimeout(() => {
        this.delayClickRestart = true
        clearTimeout(aa)
      }, 5000)
    },
    ...
    dealSpeedUpBtnViewFn: function(data) {
      ...
      if (status === 'INIT') {
        ...
        this.$root.Bus.$emit('busToShowLoginOutBtn', true) // 登录成功，显示登出按钮
        if (this.isLoginOkToAccOkStatus) {
          this.$root.Bus.$emit('busCheckLoginOkStatus', true, status)
          this.isLoginOkToAccOkStatus = false
          this.$root.Bus.$emit('busCurrentPM', true, {'mode': this.currentMode, 'pattern': this.currentPattern})
        }
        this.txtSpeedBtn = '停止加速'
        this.isShowSpeedUpPng = false
